        ...
        if(AllocaInst *AI = dyn_cast<AllocaInst>(&I)){
          if(AI->hasName()){
            if(AI->getAllocatedType()->isIntegerTy()){
              ...
              StoreInst* store = new StoreInst(ten, AI);
              store->insertAfter(AI);
            }
            if(AI->getAllocatedType()->isFloatTy()) {
                LLVMContext &context = AI->getContext();
                Value *twenty = ConstantFP::get(Type::getFloatTy(context), 20.0);
                StoreInst* store = new StoreInst(twenty, AI);
                store->insertAfter(AI);
            }
            if(AI->getAllocatedType()->isDoubleTy()) {
                LLVMContext &context = AI->getContext();
                Value *thirty = ConstantFP::get(Type::getDoubleTy(context), 30.0);
                StoreInst* store = new StoreInst(thirty, AI);
                store->insertAfter(AI);
          }
        }
      }
